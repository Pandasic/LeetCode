...
class Solution:
    def checkIfPrerequisite(self, n: int, prerequisites, queries):
        ...
        res = []
        for q in queries:
            res.append(dfs(q[0],q[1],[False for i in range(n)]))


class Solution:
    def checkIfPrerequisite(self, n: int, prerequisites, queries):
        #入 出
        M = [[set(),set()] for i in range(n)]
        for w in prerequisites:
            M[w[0]][1].add(w[1])
            M[w[1]][0].add(w[0])
            inArr = M[w[0]][0].copy()
            ...
            while(len(inArr) > 0):
                ...
                M[n][1].add(w[1])
                M[w[1]][0].add(n)
                inArr = inArr.union(M[n][0])
                inArr = inArr.difference(recode)
            print(recode)
